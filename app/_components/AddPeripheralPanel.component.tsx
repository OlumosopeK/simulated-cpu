...
type PeripheralType = "button" | "timer" | "sensor" | "proximity" | "screen" | "potentiometer" | "led" | "numpad" | "displayscreen";
...
interface FormState {
  ...
  storageAddress: string; // numpad-only
  displayscreenSourceAddress: string; // displayscreen-only
  displayscreenMode: "hex" | "ascii" | "dec" | "bin"; // displayscreen-only
}
...
const DEFAULT_FORM: FormState = {
  ...
  ledInitialLevel: "LOW",
  storageAddress: "003A",
  displayscreenSourceAddress: "003A",
  displayscreenMode: "hex",
};
...
const HANDLER_BASE: Record<PeripheralType, number> = {
  ...
  displayscreen: 0x0000, // displayscreen is output-only and does not fire interrupts
};
...
const PRESETS: Record<PeripheralType, Partial<FormState>> = {
  ...
  numpad:    { name: "Numpad", handlerAddress: "00D0", priority: "1", storageAddress: "003A" },
...
export function AddPeripheralPanel() {
  ...
  function handleSubmit() {
    ...
    if (isNaN(handlerAddress) && form.peripheralType !== "screen" && form.peripheralType !== "led" && form.peripheralType !== "displayscreen") return;
    ...
    addPeripheral({
      ...
      handlerAddress: form.peripheralType === "screen" || form.peripheralType === "led" || form.peripheralType === "displayscreen" ? 0 : handlerAddress,
      ...
      ...(form.peripheralType === "numpad" && {
        storageAddress: parseInt(form.storageAddress, 16) || 0x003A,
      }),
      ...(form.peripheralType === "displayscreen" && {
        sourceAddress: parseInt(form.displayscreenSourceAddress, 16) || 0x003A,
        displayMode: form.displayscreenMode,
      }),
    });
      ...
      {open && (
        <div className="px-3 pb-3 space-y-3">
          ...
          <div className="flex gap-1 flex-wrap">
            {(["button", "timer", "sensor", "proximity", "potentiometer", "numpad", "screen", "led", "displayscreen"] as PeripheralType[]).map((t) => (
              <button
                key={t}
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-1.5">
              {form.peripheralType !== "screen" && form.peripheralType !== "led" && form.peripheralType !== "displayscreen" && (
                <input
                  className={inputClass}
            ...
            {form.peripheralType === "numpad" && (
              <>
                <input
                  className={inputClass}
                  placeholder="Storage addr (hex)"
                  value={form.storageAddress}
                  onChange={(e) =>
                    setForm({ ...form, storageAddress: e.target.value })
                  }
                />
            ...
            {/* DisplayScreen-specific */}
            {form.peripheralType === "displayscreen" && (
              <>
                <input
                  className={inputClass}
                  placeholder="Source addr (hex)"
                  value={form.displayscreenSourceAddress}
                  onChange={(e) =>
                    setForm({ ...form, displayscreenSourceAddress: e.target.value })
                  }
                />
                <div className="flex items-center gap-2 rounded-md border border-zinc-200 bg-zinc-50 px-2 py-1.5">
                  <label className="text-[11px] text-zinc-600">Display Mode</label>
                  <select
                    value={form.displayscreenMode}
                    onChange={(e) =>
                      setForm({ ...form, displayscreenMode: e.target.value as "hex" | "ascii" | "dec" | "bin" })
                    }
                    className="ml-auto px-2 py-1 rounded border border-zinc-200 bg-white text-[11px] text-zinc-700"
                  >
                    <option value="hex">Hexadecimal (0xNN)</option>
                    <option value="ascii">ASCII Character</option>
                    <option value="dec">Decimal (0-255)</option>
                    <option value="bin">Binary (0bXXXXXXXX)</option>
                  </select>
                </div>
