...
export class ScreenPeripheral implements Peripheral {
  ...
  /**
   * Optional memory address for color control (0-15 maps to palette indices).
   * If a byte is written here, it will be used to color the display.
   * Defaults to 0x0037.
   */
  private colorAddress: number;
  ...
  setColorAddress(address: number): void {
    this.colorAddress = address;
  }
  ...
  tick(): Interrupt | null {
    ...
    // Read color index from colorAddress (0-15 maps directly to palette, >15 wraps)
    const colorByte = this.memory.read(this.colorAddress);
    const colorIdx = colorByte & 0x0F; // Take lower 4 bits for palette index
  ...
  toJSON(): PeripheralSnapshot {
    return {
      ...
      meta: {
        ...
        colorAddress: this.colorAddress,
