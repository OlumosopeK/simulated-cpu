...
import { NumpadNode } from "@/app/_components/NumpadNode.component";
import { DisplayScreenNode } from "@/app/_components/DisplayScreenNode.component";
...
const NODE_TYPES: NodeTypes = {
  ...
  led: LEDNode,
  numpad: NumpadNode,
  displayscreen: DisplayScreenNode,
};
...
function VisualizerCanvas() {
  ...
  useEffect(() => {
    ...
    const outputs = peripherals.filter(
      (p) => p.meta.type === "screen" || p.meta.type === "led" || p.meta.type === "displayscreen",
    );
    ...
    const upperNodes: Node[] = inputs.map((p, i) => {
      ...
      if (p.meta.type === "numpad") nodeType = "numpad";
    ...
    const outputNodes: Node[] = outputs.map((p, i) => ({
      id: `peripheral-${p.id}`,
      type: (p.meta.type === "screen" ? "screen" : p.meta.type === "led" ? "led" : "displayscreen") as string,
    ...
    const peripheralEdges: Edge[] = peripherals.map((p) => {
      ...
      const isDisplayScreen = p.meta.type === "displayscreen";
      const isFiring = interruptSources.includes(p.id);

      if (isScreen || p.meta.type === "led" || isDisplayScreen) {
        // Output peripherals get a data-bus edge from CPU.
        return {
          id: `peripheral-${p.id}-cpu`,
          source: "cpu",
          target: `peripheral-${p.id}`,
          label: isScreen ? "display" : isDisplayScreen ? "memory" : "output",
          type: "smoothstep",
