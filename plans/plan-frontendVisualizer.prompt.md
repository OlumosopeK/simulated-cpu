...
Create `app/page.tsx` with a plain React Flow canvas (no custom nodes). Just renders an empty flow with background grid and controls. Confirms routing + React Flow work.

## Step 2: Simulation Context
Create `app/_modules/SimulationProvider.module.tsx`. Wraps CPU + Memory instances in React context. Exposes `step()`, `start()`, `stop()`, `reset()`, and reactive state pushed via WebSockets (no polling). No UI — just the data layer.
...
## Step 3: CPU Node
Create `app/_components/CPUNode.component.tsx`. A single custom React Flow node that displays:
...
## Step 4: Memory Node
Create `app/_components/MemoryNode.component.tsx`. Custom node showing:
...
## Step 5: Controls Bar
Create `app/_components/ControlsBar.component.tsx`. Floating toolbar with:
...
Create `app/_components/PeripheralNode.component.tsx`. Generic custom node for peripherals:
- Shows name, type, status badge
- Trigger button for interactive peripherals (Button)

## Step 7: Canvas Wiring
Update `app/page.tsx` to compose all nodes with edges:
