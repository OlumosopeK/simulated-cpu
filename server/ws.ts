...
import { DisplayPeripheral } from "@/peripherals/DisplayScreen.peripheral";
import { NumpadPeripheral } from "@/peripherals/Numpad.peripheral";
...
function createPeripheral(msg: IncomingMessage): Peripheral {
  ...
  switch (peripheralType) {
    ...
    case "numpad": {
      const storageAddress = (msg.storageAddress as number) ?? 0x003A;
      return new NumpadPeripheral(
        id,
        name,
        handlerAddress,
        storageAddress,
    ...
    case "displayscreen": {
      const sourceAddress = (msg.sourceAddress as number) ?? 0x003A;
      const displayMode = (msg.displayMode as "hex" | "ascii" | "dec" | "bin") ?? "hex";
      return new DisplayPeripheral(
        id,
        name,
        memory,
        handlerAddress,
  ...
  switch (msg.type) {
    ...
    case "updatePeripheral": {
      ...
      try {
        ...
        if (peripheral instanceof TimerPeripheral) {
          ...
          if (typeof updates.colorAddress === "number") {
            peripheral.setColorAddress(updates.colorAddress);
          }
        ...
        } else if (peripheral instanceof DisplayPeripheral) {
          if (typeof updates.sourceAddress === "number") {
            peripheral.setSourceAddress(updates.sourceAddress);
          }
          if (typeof updates.displayMode === "string") {
            peripheral.setDisplayMode(updates.displayMode as "hex" | "ascii" | "dec" | "bin");
          }
        }
    ...
    case "enterNumpadValue": {
      ...
      try {
        ...
        if (peripheral instanceof NumpadPeripheral) {
          peripheral.enterValue(value);
          broadcast(buildPayload("state"));
          console.log(`[WS] Entered value ${value} (0x${value.toString(16).padStart(2, "0")}) on numpad "${id}"`);
        } else {
          throw new Error(`Peripheral "${id}" is not a numpad`);
        }
      } catch (err) {
        sendTo(ws, { type: "error", message: (err as Error).message });
      }
