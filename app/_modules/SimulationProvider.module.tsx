...
interface SimulationActions {
  ...
  addPeripheral: (opts: {
    peripheralType: "button" | "timer" | "sensor" | "proximity" | "screen" | "potentiometer" | "led" | "numpad" | "displayscreen";
    id: string;
    name: string;
    ...
    displayMode?: "hex" | "ascii" | "dec" | "bin";
  }) => void;
  removePeripheral: (id: string) => void;
  updatePeripheral: (id: string, updates: Record<string, unknown>) => void;
  setPeripheralSourceAddress: (id: string, address: number) => void;
  setPeripheralDisplayMode: (id: string, mode: "hex" | "ascii" | "dec" | "bin") => void;
  enterNumpadValue: (id: string, value: number) => void;
...
export function SimulationProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    function connect() {
      console.log(`[Simulation] Connecting to ${WS_URL}...`);
      const ws = new WebSocket(WS_URL);
      wsRef.current = ws;

      ws.onopen = () => {
        console.log("[Simulation] Connected to WS server");
        setState((prev) => ({ ...prev, connected: true }));
      };

      ws.onclose = () => {
        console.log("[Simulation] Disconnected from WS server");
  ...
  const enterNumpadValue = useCallback(
    (id: string, value: number) => {
      send({ type: "enterNumpadValue", id, value });
    },
    [send]
  );
  ...
  const setPeripheralSourceAddress = useCallback(
    (id: string, address: number) => {
      send({ type: "updatePeripheral", id, updates: { sourceAddress: address } });
    },
    [send]
  );

  const setPeripheralDisplayMode = useCallback(
    (id: string, mode: "hex" | "ascii" | "dec" | "bin") => {
      send({ type: "updatePeripheral", id, updates: { displayMode: mode } });
    },
    [send]
  );
  ...
  const value = useMemo<SimulationContextValue>(
    () => ({
      ...
      removePeripheral,
      updatePeripheral,
      setPeripheralSourceAddress,
      setPeripheralDisplayMode,
      enterNumpadValue,
      setSchedulerType,
    }),
    [
